    ...
    private void loadPhrases(String keyword) {
        new Thread(() -> {
            ...
            if (keyword == null || keyword.isEmpty()) {
                phrases = AppDatabase.getInstance(this).phraseDao().getAllPhrases().getValue();
            } else {
                phrases = AppDatabase.getInstance(this).phraseDao().searchPhrases(keyword).getValue();
            }
            if (phrases == null) phrases = new ArrayList<>();
            
            mainHandler.post(() -> adapter.setPhrases(phrases));
        }).start();
    }
